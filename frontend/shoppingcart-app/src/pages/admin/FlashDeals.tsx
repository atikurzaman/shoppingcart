...
import { Plus, Search, Edit, Trash2, Calendar, Zap, Image as ImageIcon } from 'lucide-react'
...
export default function AdminFlashDeals() {
  ...
  const { data: flashDeals = [], isLoading } = useQuery({
    queryKey: ['flash-deals', searchTerm],
    queryFn: async () => {
      const response = await api.get('/Marketing/flash-deals')
  ...
  return (
    <div className="max-w-[1600px] mx-auto animate-fade-up">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight">Flash Deals</h1>
          <p className="text-gray-500 font-medium">Create and manage limited-time discount events.</p>
        </div>
      ...
      <div className="card-premium overflow-hidden bg-white">
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="p-20 text-center text-gray-400 font-medium">Loading events...</div>
          ) : flashDeals.length === 0 ? (
            <div className="p-20 text-center">
              <div className="bg-gray-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4">
                <Zap className="w-10 h-10 text-gray-300" />
              </div>
              <h4 className="text-lg font-bold text-gray-900">No flash deals found</h4>
              <p className="text-gray-500">Wait for your first event to be created!</p>
            </div>
          ) : (
            <table className="w-full text-left">
              <thead className="bg-[#FAFAFA]">
                <tr>
                  <th className="px-8 py-4 text-xs font-black text-gray-400 uppercase tracking-widest">Event</th>
                  <th className="px-8 py-4 text-xs font-black text-gray-400 uppercase tracking-widest">Duration</th>
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-14 bg-gray-100 rounded-xl overflow-hidden border border-gray-100 flex items-center justify-center">
                           {deal.imageUrl ? <img src={deal.imageUrl} className="w-full h-full object-cover" /> : <ImageIcon className="w-6 h-6 text-gray-300" />}
                        </div>
                        <div>
                          <p className="font-bold text-gray-900">{deal.title}</p>
                          <p className="text-xs text-gray-400 font-bold uppercase tracking-tighter mt-1">{deal.products?.length || 0} Products Included</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                      <div className="flex flex-col gap-1">
                        <span className="text-xs font-bold text-gray-600 flex items-center gap-2">
                           <Calendar className="w-3 h-3 text-primary-500" /> {new Date(deal.startDate).toLocaleDateString()}
                        </span>
                        <span className="text-xs font-bold text-gray-400 flex items-center gap-2">
                           <Calendar className="w-3 h-3" /> {new Date(deal.endDate).toLocaleDateString()}
                        </span>
                      </div>
                    </td>
                    <td className="px-8 py-5">
